...
query = """SELECT * FROM hp1 UNION SELECT * FROM hp2 UNION SELECT * FROM hp3 ORDER BY MovieNumber;""" #Using SQL commands to get the union of all three movies
...
st.markdown(f"{hp123.iloc[0]['MovieName']} {hp123.iloc[0]['MovieNumber']} {hp123.iloc[0]['Character']} {hp123.iloc[0]['House']} {hp123.iloc[0]['Sentence']} Number of words: {str(hp123.iloc[0]['numWords'])}")
