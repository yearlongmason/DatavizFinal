...
with tab1:
    ...
    with col2:
        ...
        st.markdown("")
        st.markdown("")
        st.markdown('This is a great start to understanding what the rest of the data will start to look like. Of course, without surprise, Gryffindor has the most lines. This is to be expected, as the movies are first and foremost about them. The house with the second-most lines is Slytherin, and it has less than a tenth of what Gryffindor has! Aside from that, Ravenclaw has a surprising amount of representation in this movie, but not from the students. Ravenclaw lines in this movie are actually largely made up of professors such as Quirrell and Professor Flitwick. Aside from houses, muggles account for about 12% of all lines in the first movie, and Hufflepuff has no lines, which does not bode well for the diversity between houses in this movie!')
    ...
    with col2:
        ...
        st.markdown("")
        st.markdown("")
        st.markdown('These violin plots are where we take our first dive into the number of words per sentence. Instead of telling us how many total lines each house has, this metric should explain how much content is actually in each line. From these plots, we can see that while Gryffindor has a lot of lines, a lot of them do not have a lot of words in them, so it\'s very dense towards the bottom. Meanwhile, Slytherin didn\'t manage to beat Gryffindor\'s record of most words per line, but it looks like towards the top of the plot it\'s thicker than Gryffindor, which leads me to believe that while Slytherin had fewer lines than Gryffindor, it\'s possible that their lines had a lot more content to them, and I would argue the same for Ravenclaw!')
    col1, col2 = st.columns([1,4])
    with col1:
        st.markdown("")
        st.markdown("")
    ...
    st.altair_chart(numWordsPerLineHM(hp1, 1350, 400))
    res1 = st.button('New random sample')
...
with tab2:
    ...
    st.altair_chart(numWordsPerLineHM(hp2, 1350, 400))
    res2 = st.button('New random sample')
    st.markdown('This heatmap is the last visualization about the number of words per line. From the color scale on the right, it can be seen that the sentences with more words are darker, and the lighter ones have fewer words per line. This can generally be interpreted as the darker spots tending to be more dense with content, meaning more content coming from that character, which is a representation of the house they are a part of. Keep in mind that this plot comes from a random sample of 25 lines from each house, so results can vary based on the sample. Feel free to reload the page a couple times to get a feel for the data using different samples!')
    st.markdown("<h3 color: #000000;'>Conclusions</h3>", unsafe_allow_html=True)
...
with tab3:
    ...
    st.altair_chart(numWordsPerLineHM(hp3, 1350, 400))
    res3 = st.button('New random sample')
    st.markdown('This heatmap is the last visualization about the number of words per line. From the color scale on the right, it can be seen that the sentences with more words are darker, and the lighter ones have fewer words per line. This can generally be interpreted as the darker spots tending to be more dense with content, meaning more content coming from that character, which is a representation of the house they are a part of. Keep in mind that this plot comes from a random sample of 25 lines from each house, so results can vary based on the sample. Feel free to reload the page a couple times to get a feel for the data using different samples!')
    st.markdown("<h3 color: #000000;'>Conclusions</h3>", unsafe_allow_html=True)
...
with tab4:
    ...
    st.altair_chart(numWordsPerLineHM(hp123, 1350, 400))
    res4 = st.button('New random sample')
    st.markdown('This heatmap is the last visualization about the number of words per line. From the color scale on the right, it can be seen that the sentences with more words are darker, and the lighter ones have fewer words per line. This can generally be interpreted as the darker spots tending to be more dense with content, meaning more content coming from that character, which is a representation of the house they are a part of. Keep in mind that this plot comes from a random sample of 25 lines from each house, so results can vary based on the sample. Feel free to reload the page a couple times to get a feel for the data using different samples!')
    st.markdown("<h3 color: #000000;'>Conclusions</h3>", unsafe_allow_html=True)
    st.markdown('The first three movies as a whole are relatively diverse, but definitely could have been more so. Of course the story is about Gryffindor, so the representation is bound to be dominated by them, however there were definitely moments across the movies where some characters belonging to other houses got their time to shine, whether that be a character like Draco who got to shine a bit more often than a lot of the other characters, or if it were a character with some more in depth lines than the majority of the Gryffindor lines. One of the most prominent things that came from this research is how little Hufflepuff shows up in the first three movies, with only 34 lines across all 3 movies naking up less than 1% of all lines across all 3 movies! That being said, Slytherin and Ravenclaw did a pretty good job in not letting Gryffindor completely rule the story!')
